    ...
    >
      ...
      <div className="relative z-10 w-full max-w-lg rounded-t-3xl bg-slate-900 max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-800 bg-slate-900 px-5 py-4">
          <div>
            <p className="text-xs font-medium text-emerald-400">{MEAL_LABELS[meal.mealType]}</p>
        ...
        <div className="flex flex-col gap-4 p-5">
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              { label: "Kaloriya", value: `${meal.totalCalories}`, unit: "kcal", color: "text-orange-400" },
              { label: "Oqsil", value: `${meal.totalProtein}`, unit: "g", color: "text-blue-400" },
              { label: "Uglevodlar", value: `${meal.totalCarbs}`, unit: "g", color: "text-yellow-400" },
              { label: "Yog'", value: `${meal.totalFat}`, unit: "g", color: "text-red-400" },
            ].map(({ label, value, unit, color }) => (
              <div key={label} className="flex flex-col items-center rounded-2xl bg-slate-800/60 py-3">
          ...
          {meal.foods.length > 0 ? (
            <div className="flex flex-col gap-2">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Mahsulotlar</p>
